...
D = Dict(
    :ZH2 => Dict(
        ...
        :engine => :Comet,
        :tasks => [
            Dict(:id => "PepPre@2", :label => "PepPre", :dir => "Comet", :runs => 1:0.2:4, :color => missing),
            Dict(:id => "EnumInst", :label => "With Correction", :dir => "CometIso", :runs => 1:1, :color => "seagreen"),
            Dict(:id => "EnumInst", :label => "Without Correction", :dir => "Comet", :runs => 1:1, :color => "steelblue"),
        ],
    ),
    :ZH8 => Dict(
        :id => "Zubarev-Human-8",
        :engine => :Comet,
        :tasks => [
            Dict(:id => "PepPre@8", :label => "PepPre", :dir => "Comet", :runs => 1:0.2:4, :color => missing),
            Dict(:id => "EnumInst", :label => "With Correction", :dir => "CometIso", :runs => 1:1, :color => "seagreen"),
            Dict(:id => "EnumInst", :label => "Without Correction", :dir => "Comet", :runs => 1:1, :color => "steelblue"),
        ],
    ),
...
for d in [D[:ZH2], D[:ZH8]]
    ...
    for t in d[:tasks]
        ...
        for run in t[:runs]
            ...
            df = read_psm_comet(joinpath(path, t[:dir], "Comet.txt"))
            push!(t[:n_spec], read_n_spec(path, :Comet))
...
fig.savefig(joinpath(ROOT, "fig", "PepPre_number_Comet.pdf"))
plt.close(fig)
