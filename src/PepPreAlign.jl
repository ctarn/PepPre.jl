...
prepare(args) = begin
    ...
    softer = UniMZ.exp_softer(parse(Float64, args["scale"]))
...
process(path; df_ref, out, len_rt, ε_m, ε_t, bin_size, α, softer) = begin
    ...
    @showprogress for i_b in 2:(length(bins)-1)
        ...
        for i_f in bins[i_b]
            ...
            idx = filter(UniMZ.argquery_ε(df_ref.mz, a.mz, ε_m)) do i
            ...
            a.delta_mz = UniMZ.error_ppm(a.mz, b.mz)
    ...
    UniMZ.safe_save(p -> CSV.write(p, df), joinpath(out, fname * "_aligned.csv"))

    df_shift = DataFrames.DataFrame(time=Vector((bin_min:bin_max) * bin_size), shift=Δs[begin+1:end-1])
    UniMZ.safe_save(p -> CSV.write(p, df_shift), joinpath(out, fname * "_shift.csv"))
...
const DELTA_RT_MATCH = [$(join(string.(df_matched.delta_rt), ","))]
    ...
    UniMZ.safe_save(p -> write(p, html), joinpath(out, fname * ".html"))
    UniMZ.open_url(joinpath(out, fname * ".html"))
end
...
main() = begin
    ...
    paths = reduce(vcat, UniMZ.match_path.(args["data"], ".csv")) |> unique |> sort
